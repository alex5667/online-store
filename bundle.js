...
class CheckboxItem {
    ...
    element;
    checked;
    constructor(ElementSelector, category, filterType, amount, checked = false) {
        this.category = category;
        this.checkboxId = `${filterType}-${category}`;
        this.amount = amount;
        this.sectionElement = document.querySelector(ElementSelector);
        this.templateItem = document.getElementById('filter-checkbox');
    ...
    render() {
        ...
        const checkboxInput = this.element.querySelector('.filter-checkbox__input');
        const textLabel = this.element.querySelector('.filter-checkbox__text-label');
        const textAmountInCheckbox = this.element.querySelector('.filter-checkbox__text-amount-in-checkbox');
        const textAmountInProperty = this.element.querySelector('.filter-checkbox__text-amount-in-property');
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const noUiSlider = __importStar(__webpack_require__(211));
const lodash_1 = __importDefault(__webpack_require__(486));
const filter_1 = __webpack_require__(546);
...
class Filters {
    ...
    brands = productsProperties_1.brands;
    prices = productsProperties_1.prices;
    quantities = productsProperties_1.quantities;
    state;
    productsList;
    productsForFilter;
    constructor(productsList) {
        const filter = localStorage.getItem('Filter');
        this.state = filter ? JSON.parse(filter) : filter_1.STATE_FILTER;
        this.productsList = productsList;
        this.productsForFilter = products_1.default;
    }
    render() {
    ...
    resetFilters() {
        this.state = {
            ...
            price: [],
            quantity: [],
            sort: filter_1.SortFilter.DEFAULT,
            search: '',
        };
        localStorage.removeItem('Filter');
        const filters = document.querySelectorAll('.filter__items');
    ...
    sortFilters() {
        ...
        sortInput.addEventListener('change', (e) => this.setSortFilter(e.target));
    }
    setSortFilter(select) {
        const { value } = select;
        this.state = {
            ...this.state,
            sort: value,
    ...
    searchFilter() {
        ...
        const callback = (e) => this.setSearchFilter(e);
    ...
    checkboxFilters() {
        productsProperties_1.categories.forEach((category) => {
            const amount = products_1.default.filter((product) => product.category === category).length;
            new CheckboxItem_1.default('.category-filter .filter__items', category, 'category-filter', amount, this.state.category.includes(category)).element.addEventListener('change', this.setCategoryFilter.bind(this));
        });
        this.brands.forEach((brand) => {
            const amount = products_1.default.filter((product) => product.brand === brand).length;
            new CheckboxItem_1.default('.brand-filter .filter__items', brand, 'brand-filter', amount, this.state.brand.includes(brand)).element.addEventListener('change', this.setBrandFilter.bind(this));
        });
    }
    setCategoryFilter(e) {
        const checkboxLabel = e.currentTarget;
        const checkbox = e.target;
        const checkboxSpan = checkboxLabel?.querySelector('.filter-checkbox__text-label');
    ...
    setAmountInCheckbox() {
        const amountCheckboxes = document.querySelectorAll('.filter-checkbox__text-amount-in-checkbox');
        amountCheckboxes.forEach((box) => {
            const labelBox = box.closest('label');
            labelBox.classList.remove('lock-input');
            if (productsProperties_1.categories.includes(box.id)) {
            ...
            if (productsProperties_1.brands.includes(box.id)) {
    ...
    getProductsForFilter(state) {
        const { category, brand, price, quantity } = state;
        if (category.length > 0 || brand.length > 0 || price.length === 2 || quantity.length === 2) {
            this.productsForFilter = products_1.default.filter((product) => {
    ...
    rangeFilters(prop, state) {
        const selector = prop === this.prices ? `price` : `stock`;
        const Min = prop[0];
        const Max = prop[prop.length - 1];
        const DefaultMin = state[0] ? state[0] : Min;
        const DefaultMax = state[1] ? state[1] : Max;
        const Default = [DefaultMin, DefaultMax];
        const Inputs = [
            document.getElementById(`${selector}-filter__value-min`),
            document.getElementById(`${selector}-filter__value-max`),
        ];
        const Slider = document.getElementById(`${selector}-filter__slider`);
        ...
        const connectEl = Slider.querySelector('.noUi-connect');
        if (state.length === 2) {
        ...
        Slider.noUiSlider?.on('update', (values, handle) => {
            const element = Inputs[handle];
            element.innerHTML = Math.floor(+values[handle]).toString();
            const modifiedValues = [Math.floor(+values[0]), Math.floor(+values[1])];
            if (lodash_1.default.isEqual(Default, modifiedValues) && !connectEl.classList.contains('noUi-connect--unused')) {
                connectEl.classList.add('noUi-connect--unused');
            }
            else if (!lodash_1.default.isEqual(Default, modifiedValues) && connectEl.classList.contains('noUi-connect--unused')) {
                connectEl.classList.remove('noUi-connect--unused');
            }
        });
        Slider.noUiSlider?.on('update', (values) => this.setFilterRange(Min, Max, values, selector));
    }
    setFilterRange(start, end, values, selector) {
...
class ProductItem {
    ...
    isInCart;
    listeners;
    constructor(ElementSelector, product, isInCart, listeners) {
        this.product = product;
        this.listeners = listeners;
        this.sectionElement = document.querySelector(ElementSelector);
        this.templateItem = document.getElementById('product-item');
        const clonedNode = document.importNode(this.templateItem.content, true);
        this.element = clonedNode.firstElementChild;
    ...
    render() {
        const image = this.element.querySelector('.product__img img');
        const titleEl = this.element.querySelector('.product__title');
        const categoryEL = this.element.querySelector('.product__category');
        const brandEl = this.element.querySelector('.product__brand');
        const stockEL = this.element.querySelector('.product__stock');
...
class ProductsList {
    ...
    constructor(cart, listeners = []) {
        ...
        this.useFilter(filterState ? JSON.parse(filterState) : filter_1.STATE_FILTER);
    }
    render() {
        this.element.innerHTML = '';
        if (this.products.length > 0) {
            this.products.forEach((product) => new ProductItem_1.default('.products', product, this.productCart.state.includes(String(product.id)), this.listeners));
        }
    }
    useFilter(filterState) {
    ...
    searchFilter(filterState) {
        ...
        if (searchValue !== '') {
            const searchResult = this.products.filter((product) => {
                ...
                const isExist = (prop) => prop.indexOf(searchValue) > -1;
    ...
    sortFilter(filterState) {
        ...
        const sortedByPrice = () => this.products.sort((a, b) => +b.price - +a.price);
        const sortedByRating = () => this.products.sort((a, b) => +b.rating - +a.rating);
        const sortedByDiscount = () => this.products.sort((a, b) => +b.discountPercentage - +a.discountPercentage);
        switch (sort) {
            case filter_1.SortFilter.PRICE:
                this.products = sortedByPrice();
                break;
            case filter_1.SortFilter.PRICE_REVERSE:
                this.products = sortedByPrice().reverse();
                break;
            case filter_1.SortFilter.RATING:
                this.products = sortedByRating();
                break;
            case filter_1.SortFilter.RATING_REVERSE:
                this.products = sortedByRating().reverse();
                break;
            case filter_1.SortFilter.DISCOUNT:
                this.products = sortedByDiscount();
                break;
            case filter_1.SortFilter.DISCOUNT_REVERSE:
    ...
    checkboxFilter(filterState) {
        localStorage.setItem('Filter', JSON.stringify(filterState));
        const { category, brand } = filterState;
        if (category.length > 0 || brand.length > 0) {
            this.products = products_1.default.filter((product) => {
                if (category.length > 0 && !category.includes(product.category))
                    return false;
                if (brand.length > 0 && !brand.includes(product.brand))
                    return false;
    ...
    rangeFilter(filterState) {
        const { price } = filterState;
        if (price.length === 2) {
            this.products = this.products.filter((product) => (product.price >= price[0] && product.price <= price[1]));
        }
    }
}
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const MainFilters_1 = __importDefault(__webpack_require__(526));
const MainContent_1 = __importDefault(__webpack_require__(384));
const ProductsList_1 = __importDefault(__webpack_require__(846));
const Filters_1 = __importDefault(__webpack_require__(206));
class App {
    mainFilters;
    mainContent;
    productsList;
...
class Route {
    ...
    constructor() {
        this.products = products_1.default;
        this.productCart = new ProductCart_1.default();
        this.addToCartListener = ['click', this.productCart.addToCart.bind(this.productCart)];
        new App_1.default(this.productCart, [this.addToCartListener]);
            ...
            },
            {
                ...
                cb: (id) => {
                    if (id) {
                        ...
                        const product = this.products.filter((product) => product.id === +id).shift();
                        new ProductDetails_1.default(product, [this.addToCartListener]);
                    }
                },
            },
            {
    ...
    listen() {
        const inter = this.interval.bind(this);
        let handle = 0;
        clearInterval(handle);
        handle = setInterval(inter, 100);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.descriptions = exports.titles = exports.quantities = exports.prices = exports.brands = exports.categories = void 0;
const products_1 = __importDefault(__webpack_require__(926));
exports.categories = products_1.default.reduce((acc, cur) => {
    if (!acc.includes(cur.category)) {
        acc.push(cur.category);
    }
    return acc;
}, []).sort((a, b) => b.localeCompare(a));
exports.brands = products_1.default.reduce((a, c) => {
    if (!a.includes(c.brand))
        a.push(c.brand);
    return a;
}, []).sort((a, b) => b.localeCompare(a));
exports.prices = products_1.default.reduce((a, c) => {
    if (!a.includes(c.price))
        a.push(c.price);
    return a;
}, []).sort((a, b) => +a - +b);
exports.quantities = products_1.default.reduce((a, c) => {
    if (!a.includes(c.stock))
        a.push(c.stock);
    return a;
}, []).sort((a, b) => +a - +b);
exports.titles = products_1.default.reduce((a, c) => {
    if (!a.includes(c.title))
        a.push(c.title);
    return a;
}, []).sort((a, b) => b.localeCompare(a));
exports.descriptions = products_1.default.reduce((a, c) => {
    if (!a.includes(c.description))
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const route_1 = __importDefault(__webpack_require__(624));
...
class ProductDetails extends ProductItem_1.default {
    ...
    render() {
        ...
        productEl.classList.add('details');
        const allProductEl = productEl?.querySelectorAll('*');
        allProductEl.forEach((el) => el.classList.add('details'));
        const productDescription = document.querySelector('.product__description');
        const descriptionsEL = document.createElement('p');
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.STATE_FILTER = exports.SortFilter = void 0;
var SortFilter;
(function (SortFilter) {
    SortFilter["DEFAULT"] = "DEFAULT";
    SortFilter["PRICE"] = "PRICE";
    SortFilter["PRICE_REVERSE"] = "PRICE_REVERSE";
    SortFilter["RATING"] = "RATING";
    SortFilter["RATING_REVERSE"] = "RATING_REVERSE";
    SortFilter["DISCOUNT"] = "DISCOUNT";
    SortFilter["DISCOUNT_REVERSE"] = "DISCOUNT_REVERSE";
})(SortFilter = exports.SortFilter || (exports.SortFilter = {}));
exports.STATE_FILTER = {
    category: [],
    brand: [],
    price: [],
    quantity: [],
